...
def submit_exam(exam_id):
    # Always auto-score MCQs first and compute total
    try:
    ...
    try:
        cur = mysql.connection.cursor()
        cur.execute("SELECT COALESCE(SUM(score),0) as total FROM answers WHERE student_id=%s AND exam_id=%s", (session['id'], exam_id))
        total = float(cur.fetchone()['total'])
        cur.close()
    except Exception:
        total = 0.0

    # Preferred path: update exam_attempts if available
    try:
        cur = mysql.connection.cursor()
        cur.execute("SELECT status FROM exam_attempts WHERE student_id=%s AND exam_id=%s", (session['id'], exam_id))
        row = cur.fetchone()
        ...
        # Fallback path: mark completion in legacy submissions table
        try:
            cur2 = mysql.connection.cursor()
            # Ensure a row exists to indicate completion; store total score if desired
            cur2.execute(
                """
                INSERT INTO submissions (student_id, exam_id, score)
                VALUES (%s, %s, %s)
                ON DUPLICATE KEY UPDATE score=VALUES(score)
                """,
                (session['id'], exam_id, total)
            )
            mysql.connection.commit()
            cur2.close()
        except Exception:
            # If submissions table also missing, ignore and still redirect
            pass
        return jsonify({'success': True, 'redirect': url_for('student_dashboard'), 'total': total})
...
def results(exam_id):
    ...
    # Teacher/Admin view: list all students, allow grading descriptive
    cur.execute(
        """
        SELECT a.student_id, u.username, a.status, a.total_score
        FROM exam_attempts a
        JOIN users u ON u.id=a.student_id
        WHERE a.exam_id=%s AND a.status='completed'
